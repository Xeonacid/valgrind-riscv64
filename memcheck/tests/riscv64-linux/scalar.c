/* This is the riscv64 variant of memcheck/tests/x86-linux/scalar.c
...
int main(void)
{
 ...
 //SY(__NR_restart_syscall); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_fork riscv64 only has clone
   //GO(__NR_fork, "other");
   // (sse scalar_fork.c)
   ...
   // __NR_open riscv64 only as openat.
   ...
   // may not. riscv64 only has openat.
   ...
   // __NR_waitpid riscv64 only has waitid.
   //GO(__NR_waitpid, "3s 1m");
   //SY(__NR_waitpid, x0, x0+1, x0); FAIL;

   // __NR_creat riscv64 only has openat
   //GO(__NR_creat, "2s 1m");
   //SY(__NR_creat, x0, x0); FAIL;

   // __NR_link riscv64 only has linkat
   //GO(__NR_link, "2s 2m");
   //SY(__NR_link, x0, x0); FAIL;

   // __NR_unlink riscv64 only has unlinkat
   ...
   // __NR_time on riscv64 time is provided in the vdso
   //GO(__NR_time, "1s 1m");
   //SY(__NR_time, x0+1); FAIL;

   // __NR_mknod riscv64 only has mknodat
   //GO(__NR_mknod, "3s 1m");
   //SY(__NR_mknod, x0, x0, x0); FAIL;

   // __NR_chmod riscv64 only has fchmod
   //GO(__NR_chmod, "2s 1m");
   //SY(__NR_chmod, x0, x0); FAIL;

   // __NR_lchown riscv64 has no lchownat
   //GO(__NR_lchown, "n/a");
 //SY(__NR_lchown); // (Not yet handled by Valgrind) FAIL;

   // __NR_break riscv64 has no break
   //GO(__NR_break, "ni");
   //SY(__NR_break); FAIL;

   // __NR_oldstat riscv64 has no oldstat
   //GO(__NR_oldstat, "n/a");
   ...
   // __NR_umount riscv64 only has umount2
   ...
   // __NR_stime riscv64 does not have stime
   //GO(__NR_stime, "n/a");
 //SY(__NR_stime); // (Not yet handled by Valgrind) FAIL;

   // __NR_ptrace riscv64 does not use PTRACE_GETREGS
   ...
   // __NR_alarm riscv64 does not have alarm
   //GO(__NR_alarm, "1s 0m");
   //SY(__NR_alarm, x0); SUCC;

   // __NR_oldfstat riscv64 does not have oldfstat
   //GO(__NR_oldfstat, "n/a");
   // (obsolete, not handled by Valgrind)

   // __NR_pause riscv64 does not have pause
   //GO(__NR_pause, "ignore");
   // (hard to test, and no args so not much to be gained -- don't bother)

   // __NR_utime riscv64 only has utimensat
   //GO(__NR_utime, "2s 2m");
   //SY(__NR_utime, x0, x0+1); FAIL;

   // __NR_stty riscv64 doesn't have stty.
   //GO(__NR_stty, "ni");
   //SY(__NR_stty); FAIL;

   // __NR_gtty riscv64 doesm't have gtty.
   //GO(__NR_gtty, "ni");
   //SY(__NR_gtty); FAIL;

   // __NR_access riscv64 only has faccessat
   //GO(__NR_access, "2s 1m");
   //SY(__NR_access, x0, x0); FAIL;

   // __NR_nice riscv64 only has setpriority
   //GO(__NR_nice, "1s 0m");
   //SY(__NR_nice, x0); SUCC;

   // __NR_ftime riscv64 doesn't implement ftime
   //GO(__NR_ftime, "ni");
   //SY(__NR_ftime); FAIL;
   ...
   // __NR_rename riscv64 only has renameat2
   //GO(__NR_rename, "2s 2m");
   //SY(__NR_rename, x0, x0); FAIL;

   // __NR_mkdir riscv64 only has mkdirat
   //GO(__NR_mkdir, "2s 1m");
   //SY(__NR_mkdir, x0, x0); FAIL;

   // __NR_rmdir riscv64 only implements unlinkat
   //GO(__NR_rmdir, "1s 1m");
   ...
   // __NR_pipe riscv64 only has pipe2
   //GO(__NR_pipe, "1s 1m");
   ...
   // __NR_prof riscv64 doesn't implement prof
   //GO(__NR_prof, "ni");
   //SY(__NR_prof); FAIL;
   ...
   // __NR_signal riscv64 only has rt_sigaction
   //GO(__NR_signal, "n/a");
 //SY(__NR_signal); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_lock riscv64 doesn't implement lock
   //GO(__NR_lock, "ni");
   //SY(__NR_lock); FAIL;
   ...
   // __NR_mpx riscv64 doesn't implement mpx
   //GO(__NR_mpx, "ni");
   //SY(__NR_mpx); FAIL;
   ...
   // __NR_ulimit riscv64 doesn't implement ulimit
   //GO(__NR_ulimit, "ni");
   //SY(__NR_ulimit); FAIL;

   // __NR_oldolduname riscv64 doesn't implement oldolduname,
   ...
   // __NR_ustat riscv64 only has statfs
   //GO(__NR_ustat, "n/a");
   // (deprecated, not handled by Valgrind)

   // __NR_dup2 riscv64 only has dup or dup3
   //GO(__NR_dup2, "2s 0m");
   ...
   // __NR_getpgrp riscv64 only has getpgid
   ...
   // __NR_sigaction riscv64 only has rt_sigaction
   //GO(__NR_sigaction, "3s 4m");
   //SY(__NR_sigaction, x0, x0+&px[1], x0+&px[1]); FAIL;

   // __NR_sgetmask riscv64 only has rt_sigprocmask
   //GO(__NR_sgetmask, "n/a");
 //SY(__NR_sgetmask); // (Not yet handled by Valgrind) FAIL;

   // __NR_ssetmask riscv64 only has rt_sigprocmask
 ...
 //SY(__NR_ssetmask); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_sigsuspend riscv64 only has rt_sigsuspend
   // XXX: how do you use this function?
   //GO(__NR_sigsuspend, "ignore");
   // (I don't know how to test this...)

   // __NR_sigpending riscv64 only has rt_sigpending
 ...
 //SY(__NR_sethostname); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_select riscv64 only has pselect6
   //{
   ...
   // __NR_symlink riscv64 only has symlinkat
   //GO(__NR_symlink, "2s 2m");
   //SY(__NR_symlink, x0, x0); FAIL;

   // __NR_oldlstat riscv64 doesn't implement oldlstat
   //GO(__NR_oldlstat, "n/a");
   // (obsolete, not handled by Valgrind)

   // __NR_readlink riscv64 only has readlinkat
   //GO(__NR_readlink, "3s 2m");
   //SY(__NR_readlink, x0+1, x0+1, x0+1); FAIL;

   // __NR_uselib riscv64 doesn't implement uselib
   //GO(__NR_uselib, "n/a");
 ...
 //SY(__NR_reboot); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_readdir riscv64 only implements getdents64
   //GO(__NR_readdir, "n/a");
   ...
   // __NR_profil riscv64 doesn't implement profil.
   //GO(__NR_profil, "ni");
   //SY(__NR_profil); FAIL;
   ...
   // __NR_ioperm riscv64 doesn't implement ioperm
   //GO(__NR_ioperm, "3s 0m");
   //SY(__NR_ioperm, x0, x0, x0); FAIL;

   // __NR_socketcall riscv64 doesn't have socketcall.
   ...
   // __NR_stat riscv64 doesn't implement stat
   //GO(__NR_stat, "2s 2m");
   //SY(__NR_stat, x0, x0); FAIL;

   // __NR_lstat riscv64 doesn't implement lstat
   //GO(__NR_lstat, "2s 2m");
   ...
   // __NR_olduname riscv64 doesn't implement olduname
   //GO(__NR_olduname, "n/a");
   // (obsolete, not handled by Valgrind)

   // __NR_iopl riscv64 doesn't implement iopl
   //GO(__NR_iopl, "1s 0m");
   ...
   // __NR_idle riscv64 doesn't implement idle.
   //GO(__NR_idle, "ni");
   //SY(__NR_idle); FAIL;

   // __NR_vm86old riscv64 doesn't implement vm86old
   //GO(__NR_vm86old, "n/a");
 ...
 //SY(__NR_swapoff); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_ipc riscv64 has separate msgctl semctl shmctl system calls
   ...
   // __NR_sigreturn riscv64 only has rt_sigreturn
...
#define CLONE_PARENT_SETTID	0x00100000
 ...
 //SY(__NR_setdomainname); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_modify_ldt riscv64 doesn't implement modify_ldt
...
//   SY(__NR_adjtimex, x0); FAIL;
   ...
   // __NR_sigprocmask riscv64 has rt_sigprocmask
   //GO(__NR_sigprocmask, "3s 2m");
   //SY(__NR_sigprocmask, x0, x0+&px[1], x0+&px[1]); SUCC;

   // __NR_create_module riscv64 doesn't implement create_module.
 ...
 //SY(__NR_delete_module); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_get_kernel_syms riscv64 doesn't implement get_kernel_syms
   ...
   // __NR_bdflush riscv64 doesn't implement bdflush
   //GO(__NR_bdflush, "n/a");
 //SY(__NR_bdflush); // (Not yet handled by Valgrind) FAIL;

   // __NR_sysfs riscv64 doesn't handle sysfs, use proc.
   //GO(__NR_sysfs, "n/a");
 //SY(__NR_sysfs); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_afs_syscall riscv64 doesn't implement afs_syscall
   ...
   // __NR__llseek riscv64 doesn't implement _llseek
   //GO(__NR__llseek, "5s 1m");
   //SY(__NR__llseek, x0, x0, x0, x0, x0); FAIL;

   // __NR_getdents riscv64 only has getdents64
   //GO(__NR_getdents, "3s 1m");
   //SY(__NR_getdents, x0, x0, x0+1); FAIL;

   // __NR__newselect riscv64 doesn't implement _newselect
   ...
   // __NR__sysctl riscv64 doesn't implement sysctl, use /proc
 ...
 //SY(__NR_sched_rr_get_interval); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_vm86 riscv64 doesn't implement vm86
   //GO(__NR_vm86, "n/a");
   // (will probably never be handled by Valgrind)

   // __NR_query_module riscv64 doesn't implement query_module
   //GO(__NR_query_module, "ni");
   //SY(__NR_query_module); FAIL;

   // __NR_poll riscv64 only has ppoll
   //GO(__NR_poll, "3s 1m");
 ...
 //SY(__NR_rt_sigreturn); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_chown riscv64 only has fchown
   //GO(__NR_chown, "3s 1m");
   ...
   // __NR_getpmsg riscv64 doesn't implement getpmsg
 ...
 //SY(__NR_fadvise64); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_epoll_create riscv64 only has epoll_create1
   ...
   // __NR_epoll_wait riscv64 only has epoll_pwait
 ...
 //SY(__NR_tgkill); // (Not yet handled by Valgrind) FAIL;
   ...
   // __NR_utimes riscv64 only has utimensat
